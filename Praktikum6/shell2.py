...
def gapInsertionSort(data,start,gap):
    for i in range(start + gap,len(data),gap):
        currentvalue = data[i]
        position = i
        while position >= gap and data[position - gap] < currentvalue: # gunakan "<" diantara  'data[position - gap]' dan 'currentvalue' jika ingin mengurutkan dari yang paling besar ke kecil (descending)
